...
public class Payment {
	...
	public Payment(double beginningBalance, int paymentNbr, LocalDate dueDate, Loan loan, boolean bLastPayment) {
		...
		if (bLastPayment)
		{
			this.Payment = beginningBalance + loan.getEscrow();// + (beginningBalance * (loan.getInterestRate() / 12));
		}
		else
		{
			this.Payment = loan.GetPMT() + loan.getAdditionalPayment() + loan.getEscrow();
		}	
		...
		this.Principle = this.Payment - this.InterestPayment - loan.getEscrow();
	...
	public void setEscrowPayment(double amount) {
